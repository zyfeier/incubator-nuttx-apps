/****************************************************************************
 * apps/graphics/lvgl/lv_porting/lv_freetype_interface.c
 *
...
/****************************************************************************
 * Name: font_face_requester
 *
 * Description:
 *   A callback function provided by client applications.
 *   It is used by the cache manager to translate a given FTC_FaceID into a
 *   new valid FT_Face object, on demand.
 *
 * Input Parameters:
 *   face_id    - the face ID to resolve
 *   library_is - a handle to a FreeType library object.
 *   req_data   - application-provided request data
 *   aface      - a new FT_Face handle
 *
 * Returned Value:
 *   FT_Error  freeType error code. 0 means success.
 *
 ****************************************************************************/
 ...
 *
 * Description:
 *   Get a glyph's descriptor from a font when use freetype cache.
 *
 * Input Parameters:
                                   ...
                                   uint32_t unicode_letter_next)
{
...
#else
  FT_Error error = FTC_ImageCache_Lookup(image_cache, &desc_type,
                                         glyph_index, &image_glyph, NULL);
  if (error)
    {
  ...
  dsc_out->box_h = glyph_bitmap->bitmap.rows;
  dsc_out->box_w = glyph_bitmap->bitmap.width;
  dsc_out->ofs_x = glyph_bitmap->left;
...
/****************************************************************************
 * Name: get_glyph_bitmap_cb_cache
 *
 * Description:
 *   Get a glyph's bitmap from a font when use freetype cache.
 *
 * Input Parameters:
 *   font               - pointer to font.
 *   unicode_letter     - an UNICODE letter code.
...
/****************************************************************************
 * Name: get_glyph_bitmap_cb_nocache
 *
 * Description:
 *   Get a glyph's bitmap from a font when do not use freetype cache.
 *
 * Input Parameters:
 *   font               - pointer to font.
 *   unicode_letter     - an UNICODE letter code.
 ...
 *               cache instance. Use 0 for defaults.
 *   max_sizes - Maximum number of bytes to use for cached data nodes.
