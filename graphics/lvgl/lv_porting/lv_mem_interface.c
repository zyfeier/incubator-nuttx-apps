/****************************************************************************
 * apps/graphics/lvgl/lv_porting/lv_mem_interface.c
 *
...
static void lv_mem_interface_init(void)
{
  static uint32_t heap_buf[CONFIG_LV_MEM_INTERFACE_CUSTOM_SIZE
                           * 1024 / sizeof(uint32_t)];
  g_lv_heap = mm_initialize(CONFIG_LV_MEM_INTERFACE_CUSTOM_NAME,
                            heap_buf, sizeof(heap_buf));
  LV_ASSERT_NULL(g_lv_heap);
  if (g_lv_heap == NULL)
    {
      LV_LOG_ERROR("NO memory for "
                   CONFIG_LV_MEM_INTERFACE_CUSTOM_NAME
                   " heap");
    }
}
